...
\date{\today}
\publisher{Alexandre St-Aubin, Mentored by Yanees Dobberstein}
...
\begin{equation}
    ...
    \begin{equation*}
      ...
      \end{split}
    \end{equation*}
    \item[\it Log-cosh Loss.][\textit{Continuous, Differentiable}] The Log-cosh loss is given by the logarithm of the \textit{hyperbolic cosine} of the residuals between the actual value $y$ and the forecasted value $f(y_i)$. It is more computationally demanding than \textit{Huber loss}, but it provides all the same benefits without the need to establish a hyperparameter. Moreover, the log-cosh loss has the advantage of being twice differentiable, which makes it appropriate for techniques that need to solve the second derivative. It is defined as, 
  \begin{equation}
      \begin{split}
